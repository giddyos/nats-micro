...
define_error_code_enum! {
    pub enum FrameworkError {
        ...
        DeserializationError => { code: "DESERIALIZATION_ERROR", status: 502 },
        ...
        DecryptError => { code: "DECRYPT_ERROR", status: 502 },
        ...
        SignatureInvalid => { code: "SIGNATURE_INVALID", status: 401 },
        SignatureMissing => { code: "SIGNATURE_MISSING", status: 401 },
...
mod tests {
    ...
    fn status_codes_follow_http_semantics() {
        assert_eq!(FrameworkError::DeserializationError.status_code(), 502);
        assert_eq!(FrameworkError::InvalidResponse.status_code(), 502);
        assert_eq!(FrameworkError::DecryptError.status_code(), 502);
        assert_eq!(FrameworkError::SignatureInvalid.status_code(), 401);
        assert_eq!(FrameworkError::SignatureMissing.status_code(), 401);
        assert_eq!(FrameworkError::ClientConnectFailed.status_code(), 503);
        assert_eq!(TransportError::NatsRequestFailed.status_code(), 503);
        assert_eq!(TransportError::StartupError.status_code(), 503);
    }
}
