...
fn main() {
    ...
    assert_eq!(def_a.metadata.subject_prefix.as_deref(), Some("svc_a"));
    assert_eq!(def_b.metadata.subject_prefix.as_deref(), Some("svc_b"));
    assert_eq!(def_a.endpoints.len(), 1);
    assert_eq!(def_b.endpoints.len(), 1);
    assert_eq!(def_b.consumers.len(), 1);

    assert_eq!(def_a.endpoints[0].full_subject(), "svc_a.v0.a.ping");
    assert_eq!(def_b.endpoints[0].full_subject(), "svc_b.v0.b.echo");
